...
module.exports = function(path) {
    ...
    return function(req, res, next) {
        ...
        form.parse(req, function(err, fields, files) {
            ...
            var id = moment().format("YYYYMMDD-HHmmss-") + uuid.v4() + P.extname(files.file.name);
            var newPath = P.resolve(path, "./" + id); //P.resolve([from],to)把to解析为一个绝对路径
            fs.renameSync(files.file.path, newPath); //重命名（同步版的rename）
            logger.info("[UPLOAD] %s", id);
            res.end(id);
        });
    }
}
